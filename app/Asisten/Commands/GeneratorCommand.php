...
use Exception;
use Asisten\Generator\Helpers\ModelGenerator;
...
abstract class GeneratorCommand extends Command implements PromptsForMissingInput
{
    ...
    protected string $vueJsNamespace = 'resources\ts\pages';

    protected string $reactJsNamespace = 'resources\ts\views';
    ...
    public function __construct(Filesystem $files)
    {
        ...
        $this->vueJsNamespace = config('crud.vuejs.namespace', $this->vueJsNamespace);
        $this->reactJsNamespace = config('crud.reactjs.namespace', $this->reactJsNamespace);
     ...
     * @return string
     */
    protected function _getVueJsPath($name): string
    {
        return app_path($this->_getNamespacePath($this->vueJsNamespace)."{$name}.php");
    }
     ...
     * @return string
     */
    protected function _getReactJsPath($view): string
    {
        // return app_path($this->_getNamespacePath($this->reactJsNamespace)."{$name}.php");
        $name = Str::kebab(Str::plural($this->name));
        $path = match ($this->options['stack']) {
            'vuejs' => "/ts/views/pages/$name/$view.vue",
            'reactjs' => "/ts/views/pages/$name/components/{$this->name}{$view}.tsx",
     ...
     * @return string
     */
    protected function _getReactJsStorePath($view): string
    {
        // return app_path($this->_getNamespacePath($this->reactJsNamespace)."{$name}.php");
        // dd($view);
        $name = Str::kebab(Str::plural($this->name));
        ...
        $path = match ($this->options['stack']) {
            'vuejs' => "/ts/views/pages/$name/$view.vue",
            'reactjs' => "/ts/views/pages/$name/store/".(($view === 'index')?"{$view}.ts":"{$filename}{$view}.tsx"),
            default => "/views/default/$name/$view.blade.php"
        };
     ...
     * @return string
     */
    protected function _getReactJsServicesPath($view): string
    {
        // return app_path($this->_getNamespacePath($this->reactJsNamespace)."{$name}.php");
        // dd($view);
        ...
        $path = match ($this->options['stack']) {
            'vuejs' => "/ts/views/pages/$name/$view.vue",
            'reactjs' => "/ts/services/{$filename}{$view}.ts",
            default => "/views/default/$name/$view.blade.php"
        };
    ...
    protected function _getViewPath($view): string
    {
        ...
        $path = match ($this->options['stack']) {
            ...
            'vuejs' => "/ts/views/pages/$name/$view.vue",
            'reactjs' => "/ts/views/pages/$name/$view.tsx",
    ...
    protected function _getBodyStart($view): string
    {
        $name = Str::kebab($this->name);
        $path = match ($this->options['stack']) {
            'livewire' => "/views/livewire/$name/$view.blade.php",
            'vuejs' => "<div className='grid grid-cols-1 md:grid-cols-2 gap-4'>",
            'reactjs' => "<div className='grid grid-cols-1 md:grid-cols-2 gap-4'>",
            default => "</div>"
        };
    ...
    protected function _getBodyEnd($view): string
    {
        $name = Str::kebab($this->name);
        $path = match ($this->options['stack']) {
            'livewire' => "/views/livewire/$name/$view.blade.php",
    ...
    protected function getField($title, $column, string $type = 'form-field'): string
    {
        ...
        $stubs = match ($this->options['stack']) {
            'vuejs' => "vue",
            'reactjs' => "react",
            default => ""
        };
        ...
        return str_replace(
            array_keys($replace), array_values($replace), $this->getStub("views/{$stubs}/$type")
        );
    }
